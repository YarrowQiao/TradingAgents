  ...
  async start(options: StreamJsonOptions): Promise<void> {
    ...
    if (process.env.CLAUDE_PROXY_TOOLS_TRANSLATION === "1") {
      ...
      if (Object.keys(mcpServers).length > 0) {
        // Claude CLI on Windows (verified on 2.1.146) does not parse
        // inline JSON strings reliably for --mcp-config: when the arg
        // begins with `{` it gets treated as a filesystem path relative
        // to cwd, the lookup fails ("MCP config file not found"), and
        // the CLI silently runs without any MCP servers. Writing the
        // JSON to a temp file and passing the path sidesteps the
        // platform quirk and keeps the openclaw-driven config intact.
        const tmpDir = mkdtempSync(join(tmpdir(), "claude-proxy-mcp-"));
        const configPath = join(tmpDir, "mcp-config.json");
        writeFileSync(configPath, JSON.stringify({ mcpServers }), { encoding: "utf-8" });
        console.error(`[MCP] wrote ${Object.keys(mcpServers).length} server(s) to ${configPath}`);
        args.push("--mcp-config", configPath);

        // Auto-allow every tool from the registered MCP servers. Without
        // this Claude Code stops in --print mode the first time it tries
        // to call a non-allowlisted tool ("I need your permission to ...")
        // — there's no human to approve the prompt, so the analyst
        // silently degrades to "I can't access tool X" again. Pattern
        // `mcp__<server>__*` covers every tool the MCP server exposes.
        const allowPatterns = Object.keys(mcpServers).map((name) => `mcp__${name}__*`);
        args.push("--allowed-tools", allowPatterns.join(","));
        console.error(`[MCP] auto-allowed tool patterns: ${allowPatterns.join(", ")}`);
      }
    } else {
      this.mcpDecisions = [];
    }
